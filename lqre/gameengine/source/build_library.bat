...
:: --define="DEBUG=false"
:: --define="LOG=false"
java.exe -jar .\\..\\tool\\closure\\compiler.jar --compilation_level ADVANCED_OPTIMIZATIONS --define="DEBUG=true" %input% --js_output_file .\\..\\build\\library.js --create_source_map .\\..\\build\\library.map --source_map_format=V3 --generate_exports

CD .\..\build
> temp.txt (
  echo.
  for /f "usebackq delims=" %%A in (library.js) do (
