/*
	...
	uniform 
		...
		u_sampler2: sampler2D [shadow map depth] //0.0 .. 1.0 is scaled by light radius
		u_lightColour: vec3 [rgb]
		u_lightPos: vec3 [xyz]
		u_falloff: vec3 [radius, gamma, usePerspective?]
		u_projection: vec4 [near, far, right, top]
*/
DSC.Framework.Asset.Shader.Pool.PassLight8PointShadow = 
{
...
{\
	...
	float lightDistSqrd = dot(fromLight, fromLight);\
	...
	vec3 toLightNormal = fromLight / lightDistance;\
	vec2 shadowMapUV = (encodeSpheremap(toLightNormal));\
	float shadowDepth = texture2D(u_sampler2, shadowMapUV).x;\
	float lightDepth = 1.0 - (lightDistance / u_falloff.x);\
	if (lightDepth < (shadowDepth - 0.01))\
		discard;\
	...
	"m_mapUniformsNames" : {
		...
		"u_sampler2" : DSC.Framework.Asset.Shader.Uniform.FactoryRaw(DSC.Framework.Context.Uniform.s_type.TInteger),
